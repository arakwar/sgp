...
function ejcrop_initWithButtons(id, options) {
	...
	function ajaxRequest(id) {
		...
		ajaxData['image'] = $('#fichierImageBrut').val();
		...
		$.ajax({
			...
			success: function(msg) {
				if (msg != 'error') {
					// reloading image
					$('#' + id).attr('src', msg + '?ts=' + new Date().getTime());
				}
			}
		});
	}
	...
	$('body').delegate('#start_'+id,'click', function(e){
		...
		$('#start_'+id).hide();
		if (!jcrop){
			$('#'+id).Jcrop(options,function(){ 
				jcrop = this;console.log('yay2');
				jcrop.enable();
				var dim = jcrop.getBounds();
				jcrop.animateTo([dim[0]/4, dim[1]/4,dim[0]/2,dim[1]/2]);
			});
		}else{
			jcrop.enable();
			var dim = jcrop.getBounds();

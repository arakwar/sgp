...
$siPasBarrer = true; 
if($today>=$dateDepot) $siPasBarrer = true; /**************REMETTRE A FALSE POUR QUE LES DISPOS SE BARRENT*************/

 //the javascript that doing the job
 $script = "function changePage(){
             window.location = '".$this->createUrl('evenement/dispo',array('date'=>$dateParcours->format("Ymd"))).
             	"&idUsager='+document.getElementById('lstUsager').value+'&caserne='+document.getElementById('lstCaserne').value;
}";
...
<div id="tab1" class="<?php if($siPasBarrer) echo "modifiable";?>">
	...
	<div class="SSCal_header" style="clear:both;">
		<div class="enTeteDiv">
			...
			echo CHtml::link('◄',array("dispo","idUsager"=>$usager->id,"date"=>$dateParcours->format("Ymd"),"caserne"=>$caserne));
			...
			<?php if(count($tblUsager)>0 && true):?>
				...
				<div class="enTeteSec centreRRH milieu"></div>
				<div class="enTeteSec milieu"><?php echo CHtml::dropDownList('lstCaserne',$caserne,$tblCaserne,array('onChange'=>"changePage();",
		...
		<?php
			...
			do{
				...
				if($indexJour==0){
					...
					$bufferJour = "<td><div>".$jourSemaine[$indexJour]." - ".substr($jourEnCours,8,2)."</div>";
				}else{
					echo "<td><div>".$jourSemaine[$indexJour]." - ".substr($jourEnCours,8,2)."</div>";
				}
...
$(".modifiable div.caseNoire:not(.opened)").live("click",function(){ //quand les semaines sont fermée
	var ceci = $(this);
